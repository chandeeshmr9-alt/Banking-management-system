...
import type { Account, AccountSortKey, SortDirection } from "./account-types";
...
const sortableColumns: Array<{ key: AccountSortKey; label: string }> = [
      ...
      <CardHeader className="space-y-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-2">
            ...
            <CardDescription className="text-slate-600 dark:text-slate-300">Search, sort, page through, and manage account records.</CardDescription>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
            <div className="flex items-center gap-2 font-semibold text-slate-900 dark:text-slate-100">
              <Landmark className="h-4 w-4 text-primary dark:text-sky-300" />
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full lg:max-w-xl">
            ...
            <input
              ...
              placeholder="Search by account, customer, branch, type, or balance"
          ...
          <Button variant="outline" className="rounded-xl lg:w-fit" onClick={onAddAccount}>
        ...
        {showNoResults ? (
          <div className="rounded-[1.75rem] border border-dashed border-sky-200 bg-sky-50/70 p-8 text-center dark:border-slate-700 dark:bg-slate-900">
            ...
            <h3 className="mt-4 text-xl font-bold text-slate-900 dark:text-slate-100">
              {searchQuery ? "No matching accounts found" : "No account records available"}
            </h3>
            <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-600 dark:text-slate-300">
              {searchQuery
                ...
                : "Use the Add Account button to create the first local account record."}
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              ...
              <Button variant="banking" className="rounded-xl" onClick={onAddAccount}>
                <PlusCircle className="h-4 w-4" />
                Add Account
              </Button>
            </div>
                ...
                <tbody className="divide-y divide-border bg-white dark:bg-slate-900">
                  {hasRecords ? (
                    accounts.map((account) => (
                      <tr key={account.account_id} className="group transition-colors hover:bg-sky-50/60">
                        ...
                        <td className="px-5 py-4 font-semibold text-slate-900 dark:text-slate-100">{account.customer_id}</td>
                        <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{account.branch_id}</td>
                        <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{account.account_type}</td>
                        <td className="px-5 py-4 font-semibold text-slate-900 dark:text-slate-100">{formatCurrency(account.balance)}</td>
                        <td className="px-5 py-4">
                    ...
                    <tr>
                      <td colSpan={6} className="px-5 py-14 text-center text-sm text-slate-500 dark:text-slate-300">
